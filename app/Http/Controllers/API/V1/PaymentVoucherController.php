...
use App\Http\Requests\PreviewPaymentVoucherAllocationRequest;
...
class PaymentVoucherController extends BaseController
{
    public function __construct(
        protected PaymentVoucherService $paymentVouchers,
    ) {
    }
    ...
    public function index(ListPaymentVoucherRequest $request)
    {
        ...
        $query = PaymentVoucher::query()
            ->with(PaymentVoucherService::eagerLoads())
        ...
        $data = $query->get();
        $payload = collect(PaymentVoucherResource::collection($data)->resolve());
        $additionalFilters = [];

        if ($request->boolean('include_summaries', true)) {
            $additionalFilters['listSummaries'] = $this->paymentVouchers->listSummaries($data);
        }
    ...
    public function store(StorePaymentVoucherRequest $request)
    {
        ...
        try {
            $voucher = $this->paymentVouchers->create(
                $data,
                (int) $this->getTenantId(),
                (int) auth('sanctum')->id(),
            );

            return $this->responder(__('messages.api.created'), 201, new PaymentVoucherResource($voucher))->respond();
    ...
    {
        $item = PaymentVoucher::with(PaymentVoucherService::eagerLoads())->findOrFail($id);
    ...
    {
        $paymentVoucher = PaymentVoucher::findOrFail($request->payment_voucher_id);
        $data = $request->validated();
        $attachments = $request->hasFile('attachments') ? $request->file('attachments') : null;

        try {
            $voucher = $this->paymentVouchers->addPayment(
                $paymentVoucher,
                $data,
                (int) $this->getTenantId(),
                (int) auth('sanctum')->id(),
            ...
            return $this->responder(__('messages.api.updated'), 200, new PaymentVoucherResource($voucher))->respond();
        } catch (ValidationException $exception) {
            return $this->responder(__('messages.api.validation_error'), 422, [], $exception->errors())->respond();
        } catch (\Throwable $exception) {
            report($exception);
    ...
    public function allocationPreview(PreviewPaymentVoucherAllocationRequest $request)
    {
        $data = $request->validated();
        ...
        $lines = $this->paymentVouchers->allocationPreview(
        ...
        return $this->responder(__('messages.api.retrieved'), 200, [
            'supplierInvoices' => $lines,
        ])->respond();
    }

    public function prefill()
    {
        $payload = $this->paymentVouchers->prefill(request()->integer('invoice_id') ?: null);
    ...
    {
        $supplier = Supplier::query()->whereRelation('acc4', 'code', $supplier_acc4_code)->firstOrFail();

        return $this->responder(__('messages.api.retrieved'), 200, [
            'invoices' => Invoice::dropdownUnpaidForSupplier($supplier->id, false),
            'lines' => $this->paymentVouchers->allocationPreview((string) $supplier_acc4_code, 'fifo', [], 0),
        ])->respond();
    }
