...
class PurchaseReturnService
{
     ...
     */
    public function create(array $payload, int $tenantId, int $userId): PurchasesReturns
    {
        $formData = $this->normalizePayload($payload, $tenantId, $userId);
        $returnMode = $formData['return_mode'] ?? 'invoice';

        $error = PurchasesReturnsResource::validateReturnDetailsForCreate($formData);
        ...
        return DB::transaction(function () use ($formData, $returnMode, $tenantId, $userId) {
            $record = PurchasesReturns::create([
                'tenant_id' => $tenantId,
                'invoice_id' => $returnMode === 'supplier' ? null : ($formData['invoice_id'] ?? null),
                'supplier_id' => $returnMode === 'supplier'
                    ? ($formData['supplier_id'] ?? null)
                    : (Invoice::find($formData['invoice_id'] ?? null)?->supplier_id),
            ...
            if ($returnMode === 'invoice') {
                foreach ($formData['details'] ?? [] as $detail) {
                    $record->details()->create([
                        ...
                        'tenant_id' => $tenantId,
                        'purchases_returns_id' => $record->id,
                        'user_id' => $userId,
                    ]);
                }
            }
            ...
            SalesReturnWorkflow::settlePurchaseReturnPayment($record, $formData, $returnTotal);
            ...
            return $record->fresh(['details.invoiceItem', 'invoice.supplier', 'supplier', 'user']);
        });
    }
    ...
    public function normalizePayload(array $payload, int $tenantId, int $userId): array
    {
        ...
        $details = $this->buildDetails(
            ...
            $detailsInput ?? [],
            $pricesIncludesTaxes,
            (int) ($payload['supplierId'] ?? $payload['supplier_id'] ?? 0),
        );
        ...
        if ($returnMode === 'invoice') {
            ...
            if (! $invoiceId) {
                throw ValidationException::withMessages([
                    'invoiceNo' => __('fields.purchase_return_invoice_required'),
                ]);
            }
        }
     ...
     */
    protected function processCreditRefund(PurchasesReturns $record, array $formData, float $returnTotal): void
    {
        if (($formData['payment_terms'] ?? 'cash') !== 'credit') {
            return;
        }
        ...
        ReturnPaymentTermsService::instance()->recordPurchaseReturnCreditRefund($record, [
            'amount' => $amount,
