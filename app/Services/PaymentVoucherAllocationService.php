...
class PaymentVoucherAllocationService
{
    ...
    public function unpaidPurchaseInvoicesForSupplier(int $supplierId): Collection
    {
    ...
    public function unpaidPurchaseInvoicesForAcc4Code(int $acc4Code): Collection
    {
        ...
        return $this->unpaidPurchaseInvoicesForSupplier((int) $supplierId);
    }
    ...
    public function allocate(float $amount, Collection $invoices, string $mode, array $selectedInvoiceIds = []): array
    {
        ...
        if ($mode === 'selected') {
            ...
            if ($selectedInvoiceIds === []) {
                throw ValidationException::withMessages([
                    'data.supplier_invoices' => __('fields.receipt_voucher_select_at_least_one_invoice'),
                ]);
            }
        ...
        if ($pool->isEmpty()) {
            throw ValidationException::withMessages([
                'data.supplier_invoices' => __('fields.receipt_voucher_no_unpaid_invoices'),
            ]);
        }
