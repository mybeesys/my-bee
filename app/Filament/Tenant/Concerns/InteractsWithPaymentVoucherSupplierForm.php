...
use App\Filament\Tenant\Resources\PaymentVoucherResource\Pages\CreatePaymentVoucher;
use App\Filament\Tenant\Resources\PaymentVoucherResource\Pages\EditPaymentVoucher;
use App\Models\Acc4;
...
trait InteractsWithPaymentVoucherSupplierForm
{
    public static function refreshSupplierInvoiceLines(object $livewire, ?Set $set = null): void
    {
        ...
        $paidAmount = static::normalizePaymentPaidAmount($livewire->data['paid_amount'] ?? 0);
        $selectedIds = collect($livewire->data['supplier_invoices'] ?? [])
        ...
        $lines = PaymentVoucherAllocationService::instance()->buildInvoiceLineStates(
            $invoices,
            $mode,
            $selectedIds,
            $paidAmount,
        ...
        $livewire->data['supplier_invoices'] = $lines;
    ...
    public static function normalizePaymentPaidAmount(mixed $value): float
    {
        if (is_numeric($value)) {
    ...
    protected static function supplierPaymentCreateSection(): Forms\Components\Section
    {
        return Forms\Components\Section::make()
            ->visible(fn ($livewire, Get $get) => $livewire instanceof CreatePaymentVoucher && $get('for') === 'supplier')
            ->schema([
                ...
                Forms\Components\Grid::make(2)->schema([
                    ...
                    TextInput::make('paid_amount')
                        ...
                        ->minValue(0.01)
                        ->live(onBlur: true)
                        ->afterStateUpdated(fn ($livewire) => static::refreshSupplierInvoiceLines($livewire))
                        ->required(),
                ...
                Forms\Components\Placeholder::make('supplier_invoices_hint')
                    ->hiddenLabel()
                    ->content(new HtmlString(
                        '<p class="text-sm text-gray-500">' . e(__('fields.payment_voucher_supplier_invoices_hint')) . '</p>'
                    )),

                TableRepeater::make('supplier_invoices')
