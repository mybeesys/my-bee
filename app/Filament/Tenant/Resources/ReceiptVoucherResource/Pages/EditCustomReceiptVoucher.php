<?php
    ...
    class EditCustomReceiptVoucher extends Page implements HasActions
    {
        use InteractsWithForms, InteractsWithFormActions;

        protected static string $resource = ReceiptVoucherResource::class;

        protected static string $view = 'filament.resources.receipt-voucher-resource.pages.edit-custom-receipt-voucher';

        public $receipt_voucher_id, $no, $date, $client_id, $account_id, $invoice_id, $order_id, $order_no, $delivery, $delivery_extra, $driver_id, $payments, $total_price_sdg, $total_price_usd;

        public $invoice_total_sdg, $invoice_total_usd, $invoice_paid_sdg, $invoice_paid_usd;

        public $invoice, $order;

        public $payment_method, $cheque_holder_name, $cheque_status,
            $bank_transfer_reference_no, $currency_id, $amount,
            $exchange_rate, $amount_sdg, $statement, $files;
        ...
        public function mount(): void
        {
            ...
            $this->receipt_voucher_id = \Route::current()->parameter('record');

            $receiptVoucher = ReceiptVoucher::findOrFail($this->receipt_voucher_id);

            $this->invoice_id = $receiptVoucher->invoice_id;
            ...
            if ($this->invoice_id) {
                $invoice = Invoice::with(['items', 'payments'])->whereId($this->invoice_id)->firstOrFail();
                ...
                foreach ($this->invoice->payments as $payment) {
                    $data = [
                        ...
                        'cheque_holder_name' => $payment->cheque_holder_name,
                        'cheque_status' => $payment->cheque_status,
                        'bank_transfer_reference_no' => $payment->bank_transfer_reference_no,
                    ...
                    $this->payments[Str::random(9)] = $data;
                }
            }
...
//                                    ->afterStateHydrated(fn(Set $set) => $set('no', generate_receipt_voucher()))
                                ...
                                Select::make('invoice_id')
                                    ...
                                    ->options(function (Get $get) {
                                        return Invoice::dropdownUnpaid($this->client_id, true, [$this->invoice_id]);
                                    })
