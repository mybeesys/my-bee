...
use App\Filament\Tenant\Concerns\InteractsWithOrderActions;
    ...
    public static function table(Tables\Table $table): Tables\Table
    {
        return $table
            ...
            ->columns([
                ...
                Tables\Columns\TextColumn::make('invoice.payment_state')
                    ->label(__('fields.invoice_status'))
                    ->badge()
                    ->toggleable()
                    ->getStateUsing(function (Order $record) {
                        ...
                        if ($record->invoice->status === 'cancelled') {
                            return __('fields.invoice_status_cancelled');
                        }

                        return $record->invoice->payment_status;
                    })
                    ->color(function (Order $record) {
                        if (! $record->invoice) {
                            return 'gray';
                        ...
                        return match ($record->invoice->settlement_status_key) {
                            'due' => 'gray',
                            'partial' => 'warning',
                            'cash', 'paid' => 'success',
                            default => 'gray',
...
//                    ->sortable(),
            ...
            ->actions([
                static::configureOrderTableActionGroup(Tables\Actions\ActionGroup::make([
                    static::makeOrderViewTableAction(),
                    static::makeOrderChangeStatusTableAction(),
                    static::makeReviewSalesInvoiceTableAction(),
                    static::makeConfirmSalesInvoiceTableAction(),
                    static::makeCompletePaymentTableAction(),
                ])),
            ])
            ->bulkActions([
            ]);
