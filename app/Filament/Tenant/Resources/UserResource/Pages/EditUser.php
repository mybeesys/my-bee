...
class EditUser extends EditRecord
{
        ...
        return [
            ActionGroup::make([
                \Filament\Actions\Action::make('mark_active_inactive')
                    ...
                    ->action(function () {
                        if (auth()->id() == $this->record->id || $this->record->isClient()) {
                            Notification::make()
                ...
                \Filament\Actions\Action::make('reset_password')
                    ...
                    ->modalDescription('Are you sure you\'d like to reset the user\'s password?')
                    ->modalSubmitActionLabel('Yes, reset account')
                    ->icon('heroicon-o-lock-open')
                    ->action(function (array $data) {
                        try {
                            abort_if($this->record->isClient() and !auth()->user()->isClient(), 403, 'Only a super admin can perform this action');
                            abort_if(!auth()->user()->isClient(), 403, 'Only a super admin can perform this action');
                ...
                \Filament\Actions\Action::make('assign_roles')
                    ...
                    ->color('primary')
                    ->visible(fn() => auth()->user()->hasRole(User::ROLE_CLIENT))
                    ->mountUsing(function (ComponentContainer $form) {
                        $roles = Role::where('tenant_id', filament()->getTenant()->id)
                            ->whereIn('name', $this->record->roles->pluck('name')->toArray())
                            ->pluck('name')
                            ->toArray();
                    ...
                    ->form(function () {
                        ...
                        RoleService::instance()->syncRoles($this->record, $data['roles']);
                        fns()->saved();
                    }),
    ...
    protected function mutateFormDataBeforeFill(array $data): array
    {
        ...
        if (empty($data['roles_list']))
            $data['roles_list'] = "No roles found!";
